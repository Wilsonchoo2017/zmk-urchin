...
## ARTSEY one-handed typing mode

The left half can double as a one-handed typing surface using the
[ARTSEY](https://artsey.io) 8-key combo layout: eight keys (two rows of four,
on the left half) type every letter through key combinations.

Adapted from [artseyio/zmk-artsey](https://github.com/artseyio/zmk-artsey)
(`config/artsey.dtsi`), ported into `config/artsey.dtsi` + wired up from
`config/urchin.keymap`.

### Turning it on

It is a normal layer toggle, so it does not affect day-to-day typing:

* **On:** `SETTINGS` layer, top row, second key -> `&tog ARTSEY`
  (the same key is also on the ARTSEY layer's bottom-left thumb to leave it)
* Normal typing resumes as soon as the layer is toggled off.

### The 8 keys

Rows `S T R A` / `O I Y E`, using columns 0-3 of the left half. `A` and `E` sit
under the index finger, as ARTSEY requires for left-handed use.

* **Tap** a key for its letter.
* **Hold** a key for its sub-layer (numbers, symbols, parens, custom).
* **Multiple keys together** = a combo (this is where most letters live).

Sub-layers: `ARTSEY num` (7), `ARTSEY sym` (8), `ARTSEY paren` (9),
`ARTSEY nav` (10), `ARTSEY BT` (11), `ARTSEY misc` (12). Layer ids 6-12.

### Notes for anyone editing this

* Every ARTSEY combo carries `layers = <...>` so it only fires on its own
  layer. Upstream leaves most combos un-gated because upstream firmware *is*
  ARTSEY; here they would otherwise hijack normal typing.
* The ARTSEY layer ids are fixed by **child order** in the `keymap` node: the
  six normal layers must stay above the `#include "artsey.dtsi"` at the bottom
  of `urchin.keymap`.
* `config/artsey.dtsi` deliberately does **not** include `behaviors.dtsi`,
  `keys.h`, `bt.h` or `outputs.h`: none has an include guard, and re-including
  them would redefine every labelled behaviour node.
* Prebuilt `firmware/artsey/*.uf2` were produced with the same
  `zmkfirmware/zmk-build-arm:stable` container the CI uses.

### Diagrams

Regenerate after any keymap edit (needs [keymap-drawer](https://github.com/caksoylar/keymap-drawer)):

```sh
uvx --from keymap-drawer keymap parse -c 10 -z config/urchin.keymap -o /tmp/ke.yaml
uvx --from keymap-drawer keymap -c draw.yaml draw -j config/urchin.json /tmp/ke.yaml -o urchin-keymap.svg
rsvg-convert -f pdf -o urchin-keymap.pdf urchin-keymap.svg
```

* `urchin-keymap.svg` / `.pdf` - all 13 layers, every combo.
* `artsey-layer-reference.svg` / `.pdf` - the 7 ARTSEY layers only.

`draw.yaml` sets `n_columns: 2` (the repo's layout), `key_w/key_h: 50`, and
`separate_combo_diagrams: true` - ARTSEY puts 42 combos on 8 keys, so the
default combo overlay is unreadable and each combo gets its own mini-diagram.

#### A4 printable pages

The SVGs above are one tall image, which does not print usefully. For paper, split
them across real A4 pages:

```sh
python3 scripts/a4_split.py            # -> docs/*-a4.pdf (3 pages each)
```

It cuts **only between rows of panels**, so no layer panel or combo diagram is
ever split across a page boundary. Pages are 210x297 mm with 10 mm margins and a
header strip showing `page N of M` plus the layers on that page. Needs
`rsvg-convert`, `pdfunite` and `pdftoppm`.

Output is **greyscale** by default: the pages are meant to be printed, and
keymap-drawer's colour accents (`#cdf` combo boxes, `#fdd` held keys) sit at
almost identical luminance — desaturated naively they become the *same* grey and
you lose the difference between "this is a combo" and "this key is held". The
print palette spaces them out instead:

* keycap `#ffffff` - no toner for an empty cap
* combo box `#cccccc` - light grey
* held key `#8f8f8f` - dark grey
* outlines `#808080` instead of the barely-visible `#c9cccf`

Each build re-rasterises the PDF it just wrote and fails if any pixel is not
neutral, or if an expected key colour never hit the palette. Pass `--colour` to
keep the original colours; `--margin MM` and `--out DIR` also apply.
